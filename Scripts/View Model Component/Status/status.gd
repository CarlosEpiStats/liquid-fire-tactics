...
signal status_added(StatusEffect)
signal status_removed(StatusEffect)
...
func remove(target: StatusCondition):
	...
	condition.assign(children.filter(func(node): return node is StatusCondition and not node.is_queued_for_deletion()))
